cla_secsse_loglik_rhs <- function(t, y, parameter) {
    ...
    dD <- -((unlist(lapply(lambdas, sum))) + mus + Q %*% (rep(1, d))) *
...
cla_doParalThing <- function(take_ancesSub,
    ...
    statesNEW <- foreach::foreach(ii = 1:2,
                                  .packages = c("secsse",
                                                ...
                                                "geiger",
                                                "apTreeshape"),
                                  .export = c("cla_secsse_loglik",
                                              "ode_FORTRAN",
                                              "cla_calThruNodes"))
                   %dopar% {
        ancesSub <- take_ancesSub[[ii]]
        for (i in 1:length(ancesSub)) {
            calcul <- cla_calThruNodes(ancesSub[i],
                                       states,
                                       loglik,
                                       forTime,
                                       parameter,
    ...
    if (run_parallel == TRUE) {
        if (is.null(setting_calculation)) {
            check_input(traits,
                        ...
                        is_complete_tree)
            setting_calculation <-
              build_initStates_time_bigtree(phy,
                                            traits,
                                            num_concealed_states,
                                            sampling_fraction,
                                            is_complete_tree,
        ...
        for (i in 1:length(ancesRest)) {
            calcul <- cla_calThruNodes(ancesRest[i],
                                       states,
                                       loglik,
                                       forTime,
                                       parameter,
